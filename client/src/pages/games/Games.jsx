...
const GamesContent = () => {
  ...
  const fetchAllGames = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // Fetch external game data if available
        if (gamesData.length > 0) {
          const gameApiIDs = gamesData.map(game => game.gameApiID).filter(id => id);

          if (gameApiIDs.length > 0) {
            try {
              ...
              if (externalApiResponse.data) {
                ...
                gamesData = gamesData.map(localGame => {
                  const externalGame = externalGames.find(eg => eg._id === localGame.gameApiID);

                  if (externalGame) {
                    const { category, provider, ...externalGameWithoutCategory } = externalGame;
                    return {
                      ...localGame,
              ...
              console.error("Error fetching from external games API:", apiError);
            }
          }
        }
  ...
  }, [searchTerm, selectedProviders, selectedCategory, selectedGameTypes, selectedThemes, selectedSpecialFeatures, sortOption, games]);
  ...
  const handleOpenGame = async (game) => {
    ...
    try {
      ...
      // Check if game already has game_uuid from external API
      if (game.game_uuid) {
        console.log("Game already has UUID, navigating to:", game.game_uuid);
        navigate(`/game/${game.game_uuid}`);
        return;
      }

      // If no game_uuid, try to fetch from your backend
